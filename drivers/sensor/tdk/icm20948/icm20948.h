/**
 * sensor/icm20948/icm20948.h - Header file for ICM20948 sensor driver
 */

/**
 * copyright 2025, [name of author]
 * SPDX-License-Identifier: Apache-2.0
 */



#ifndef ZEPHYR_DRIVERS_SENSOR_ICM20948_ICM20948_H_
#define ZEPHYR_DRIVERS_SENSOR_ICM20948_ICM20948_H_

#include <zephyr/drivers/sensor.h>
#include <zephyr/types.h>
#include <zephyr/drivers/gpio.h>
#include <zephyr/kernel.h>
#include <zephyr/sys/util.h>


#if DT_ANY_INST_ON_BUS_STATUS_OKAY(spi)
#include <zephyr/drivers/spi.h>
#endif /* DT_ANY_INST_ON_BUS_STATUS_OKAY(spi) */


#if DT_ANY_INST_ON_BUS_STATUS_OKAY(i2c)
#include <zephyr/drivers/i2c.h>
#endif /* DT_ANY_INST_ON_BUS_STATUS_OKAY(i2c) */


#define ICM20948_BANK_0                         0x00
#define ICM20948_BANK_1                         0x01
#define ICM20948_BANK_2                         0x02
#define ICM20948_BANK_3                         0x03


/* USER BANK 0 REGISTERS */
#define ICM20948_WHO_AM_I                       0x00
#define ICM20948_USER_CTRL                      0x03
#define ICM20948_LP_CONFIG                      0x05
#define ICM20948_PWR_MGMT_1                     0x06
#define ICM20948_PWR_MGMT_2                     0x07
#define ICM20948_INT_PIN_CFG                    0x0F
#define ICM20948_INT_ENABLE                     0x10
#define ICM20948_INT_ENABLE_1                   0x11
#define ICM20948_INT_ENABLE_2                   0x12
#define ICM20948_INT_ENABLE_3                   0x13
#define ICM20948_I2C_MST_STATUS                 0x17
#define ICM20948_INT_STATUS                     0x19
#define ICM20948_INT_STATUS_1                   0x1A
#define ICM20948_INT_STATUS_2                   0x1B
#define ICM20948_INT_STATUS_3                   0x1C
#define ICM20948_DELAY_TIMEH                    0x28
#define ICM20948_DELAY_TIMEL                    0x29
#define ICM20948_ACCEL_XOUT_H                   0x2D
#define ICM20948_ACCEL_XOUT_L                   0x2E
#define ICM20948_ACCEL_YOUT_H                   0x2F
#define ICM20948_ACCEL_YOUT_L                   0x30
#define ICM20948_ACCEL_ZOUT_H                   0x31
#define ICM20948_ACCEL_ZOUT_L                   0x32
#define ICM20948_GYRO_XOUT_H                    0x33
#define ICM20948_GYRO_XOUT_L                    0x34
#define ICM20948_GYRO_YOUT_H                    0x35
#define ICM20948_GYRO_YOUT_L                    0x36
#define ICM20948_GYRO_ZOUT_H                    0x37
#define ICM20948_GYRO_ZOUT_L                    0x38
#define ICM20948_TEMP_OUT_H                     0x39
#define ICM20948_TEMP_OUT_L                     0x3A
#define ICM20948_EXT_SLV_SENS_DATA_00           0x3B
#define ICM20948_EXT_SLV_SENS_DATA_01           0x3C
#define ICM20948_EXT_SLV_SENS_DATA_02           0x3D
#define ICM20948_EXT_SLV_SENS_DATA_03           0x3E
#define ICM20948_EXT_SLV_SENS_DATA_04           0x3F
#define ICM20948_EXT_SLV_SENS_DATA_05           0x40
#define ICM20948_EXT_SLV_SENS_DATA_06           0x41
#define ICM20948_EXT_SLV_SENS_DATA_07           0x42
#define ICM20948_EXT_SLV_SENS_DATA_08           0x43
#define ICM20948_EXT_SLV_SENS_DATA_09           0x44
#define ICM20948_EXT_SLV_SENS_DATA_10           0x45
#define ICM20948_EXT_SLV_SENS_DATA_11           0x46
#define ICM20948_EXT_SLV_SENS_DATA_12           0x47
#define ICM20948_EXT_SLV_SENS_DATA_13           0x48
#define ICM20948_EXT_SLV_SENS_DATA_14           0x49
#define ICM20948_EXT_SLV_SENS_DATA_15           0x4A
#define ICM20948_EXT_SLV_SENS_DATA_16           0x4B
#define ICM20948_EXT_SLV_SENS_DATA_17           0x4C
#define ICM20948_EXT_SLV_SENS_DATA_18           0x4D
#define ICM20948_EXT_SLV_SENS_DATA_19           0x4E
#define ICM20948_EXT_SLV_SENS_DATA_20           0x4F
#define ICM20948_EXT_SLV_SENS_DATA_21           0x50
#define ICM20948_EXT_SLV_SENS_DATA_22           0x51
#define ICM20948_EXT_SLV_SENS_DATA_23           0x52
#define ICM20948_FIFO_EN_1                      0x66
#define ICM20948_FIFO_EN_2                      0x67
#define ICM20948_FIFO_RST                       0x68
#define ICM20948_FIFO_MODE                      0x69
#define ICM20948_FIFO_COUNTH                    0x70
#define ICM20948_FIFO_COUNTL                    0x71
#define ICM20948_FIFO_R_W                       0x72
#define ICM20948_DATA_RDY_STATUS                0x74
#define ICM20948_FIFO_CFG                       0x76
#define ICM20948_REG_BANK_SEL                   0x7F

#define ICM20948_I2C_MST_STATUS_SLV4_DONE        BIT(6)
#define ICM20948_I2C_MST_STATUS_SLV4_NACK        BIT(4)

/* USER BANK 1 REGISTERS */
#define ICM20948_SELF_TEST_X_GYRO               0x02
#define ICM20948_SELF_TEST_Y_GYRO               0x03
#define ICM20948_SELF_TEST_Z_GYRO               0x04
#define ICM20948_SELF_TEST_X_ACCEL              0x0E
#define ICM20948_SELF_TEST_Y_ACCEL              0x0F
#define ICM20948_SELF_TEST_Z_ACCEL              0x10
#define ICM20948_XA_OFFS_H                      0x14
#define ICM20948_XA_OFFS_L                      0x15
#define ICM20948_YA_OFFS_H                      0x17
#define ICM20948_YA_OFFS_L                      0x18
#define ICM20948_ZA_OFFS_H                      0x1A
#define ICM20948_ZA_OFFS_L                      0x1B
#define ICM20948_TIMEBASE_CORRECTION_PLL        0x28

/* USER BANK 2 REGISTERS */
#define ICM20948_GYRO_SMPLRT_DIV                0x00
#define ICM20948_GYRO_CONFIG_1                  0x01
#define ICM20948_GYRO_CONFIG_2                  0x02
#define ICM20948_XG_OFFS_USRH                   0x03
#define ICM20948_XG_OFFS_USRL                   0x04
#define ICM20948_YG_OFFS_USRH                   0x05
#define ICM20948_YG_OFFS_USRL                   0x06
#define ICM20948_ZG_OFFS_USRH                   0x07
#define ICM20948_ZG_OFFS_USRL                   0x08
#define ICM20948_ODR_ALIGN_EN                   0x09
#define ICM20948_ACCEL_SMPLRT_DIV_1             0x10
#define ICM20948_ACCEL_SMPLRT_DIV_2             0x11
#define ICM20948_ACCEL_INTEL_CTRL               0x12
#define ICM20948_ACCEL_WOM_THR                  0x13
#define ICM20948_ACCEL_CONFIG                   0x14
#define ICM20948_ACCEL_CONFIG_2                 0x15
#define ICM20948_PRGM_STRT_ADDRH                0x50
#define ICM20948_PRGM_STRT_ADDRL                0x52
#define ICM20948_FSYNC_CONFIG                   0x52
#define ICM20948_TEMP_CONFIG                    0x53
#define ICM20948_MOD_CTRL_USR                   0x54

/* USER BANK 3 REGISTERS */
#define ICM20948_I2C_MST_ODR_CONFIG             0x00
#define ICM20948_I2C_MST_CTRL                   0x01
#define ICM20948_I2C_MST_DELAY_CTRL             0x02
#define ICM20948_I2C_SLV0_ADDR                  0x03
#define ICM20948_I2C_SLV0_REG                   0x04
#define ICM20948_I2C_SLV0_CTRL                  0x05
#define ICM20948_I2C_SLV0_DO                    0x06
#define ICM20948_I2C_SLV1_ADDR                  0x07
#define ICM20948_I2C_SLV1_REG                   0x08
#define ICM20948_I2C_SLV1_CTRL                  0x09
#define ICM20948_I2C_SLV1_DO                    0x0A
#define ICM20948_I2C_SLV2_ADDR                  0x0B
#define ICM20948_I2C_SLV2_REG                   0x0C
#define ICM20948_I2C_SLV2_CTRL                  0x0D
#define ICM20948_I2C_SLV2_DO                    0x0E
#define ICM20948_I2C_SLV3_ADDR                  0x0F
#define ICM20948_I2C_SLV3_REG                   0x10
#define ICM20948_I2C_SLV3_CTRL                  0x11
#define ICM20948_I2C_SLV3_DO                    0x12
#define ICM20948_I2C_SLV4_ADDR                  0x13
#define ICM20948_I2C_SLV4_REG                   0x14
#define ICM20948_I2C_SLV4_CTRL                  0x15
#define ICM20948_I2C_SLV4_DO                    0x16
#define ICM20948_I2C_SLV4_DI                    0x17

#define ICM20948_I2C_SLV4_CTRL_EN               BIT(7)


/* AK09916 MAGNETOMETER REGISTERS */
/* These registers are accessed through the ICM-20948's I2C master interface */
#define AK09916_I2C_ADDR                        0x0C    /* AK09916 I2C address */
#define AK09916_WIA1                            0x00    /* Company ID register (fixed value 0x48) */
#define AK09916_WIA2                            0x01    /* Device ID register (fixed value 0x09) */
#define AK09916_CHIP_ID                         0x09    /* Expected value from WIA2 register */
#define AK09916_ST1                             0x10    /* Status 1 register */
#define AK09916_HXL                             0x11    /* X-axis measurement data lower 8 bits */
#define AK09916_HXH                             0x12    /* X-axis measurement data higher 8 bits */
#define AK09916_HYL                             0x13    /* Y-axis measurement data lower 8 bits */
#define AK09916_HYH                             0x14    /* Y-axis measurement data higher 8 bits */
#define AK09916_HZL                             0x15    /* Z-axis measurement data lower 8 bits */
#define AK09916_HZH                             0x16    /* Z-axis measurement data higher 8 bits */
#define AK09916_ST2                             0x18    /* Status 2 register */
#define AK09916_CNTL2                           0x31    /* Control 2 register */
#define AK09916_CNTL3                           0x32    /* Control 3 register */
#define AK09916_TS1                             0x33    /* Test 1 register (DO NOT ACCESS) */
#define AK09916_TS2                             0x34    /* Test 2 register (DO NOT ACCESS) */

/* AK09916 Status Register Bit Definitions */
#define AK09916_ST1_DRDY                        0x01    /* Data Ready bit in ST1 */
#define AK09916_ST1_DOR                         0x02    /* Data Overrun bit in ST1 */
#define AK09916_ST2_HOFL                        0x08    /* Magnetic sensor overflow bit in ST2 */

/* AK09916 Operation Modes for CNTL2 Register */
#define AK09916_MODE_POWER_DOWN                 0x00    /* Power-down mode */
#define AK09916_MODE_SINGLE                     0x01    /* Single measurement mode */
#define AK09916_MODE_CONT_1                     0x02    /* Continuous measurement mode 1 (10Hz) */
#define AK09916_MODE_CONT_2                     0x04    /* Continuous measurement mode 2 (20Hz) */
#define AK09916_MODE_CONT_3                     0x06    /* Continuous measurement mode 3 (50Hz) */
#define AK09916_MODE_CONT_4                     0x08    /* Continuous measurement mode 4 (100Hz) */
#define AK09916_MODE_SELF_TEST                  0x10    /* Self-test mode */

/* AK09916 Control 3 Register Bit Definitions */
#define AK09916_CNTL3_SRST                      0x01    /* Soft reset bit */

/* Legacy magnetometer register definitions (for backward compatibility) */
#define ICM20948_WIA                            AK09916_WIA
#define ICM20948_ST1                            AK09916_ST1
#define ICM20948_HXL                            AK09916_HXL
#define ICM20948_HXH                            AK09916_HXH
#define ICM20948_HYL                            AK09916_HYL
#define ICM20948_HYH                            AK09916_HYH
#define ICM20948_HZL                            AK09916_HZL
#define ICM20948_HZH                            AK09916_HZH
#define ICM20948_ST2                            AK09916_ST2
#define ICM20948_CNTL2                          AK09916_CNTL2
#define ICM20948_CNTL3                          AK09916_CNTL3
#define ICM20948_TS1                            AK09916_TS1
#define ICM20948_TS2                            AK09916_TS2


/* AK09916 Status Register Bit Definitions */
#define AK09916_ST1_DRDY                        0x01    /* Data Ready bit in ST1 */
#define AK09916_ST1_DOR                         0x02    /* Data Overrun bit in ST1 */
#define AK09916_ST2_HOFL                        0x08    /* Magnetic sensor overflow bit in ST2 */

/* AK09916 Operation Modes for CNTL2 Register */
#define AK09916_MODE_POWER_DOWN                 0x00    /* Power-down mode */
#define AK09916_MODE_SINGLE                     0x01    /* Single measurement mode */
#define AK09916_MODE_CONT_1                     0x02    /* Continuous measurement mode 1 (10Hz) */
#define AK09916_MODE_CONT_2                     0x04    /* Continuous measurement mode 2 (20Hz) */
#define AK09916_MODE_CONT_3                     0x06    /* Continuous measurement mode 3 (50Hz) */
#define AK09916_MODE_CONT_4                     0x08    /* Continuous measurement mode 4 (100Hz) */
#define AK09916_MODE_SELF_TEST                  0x10    /* Self-test mode */

/* AK09916 Control 3 Register Bit Definitions */
#define AK09916_CNTL3_SRST                      0x01    /* Soft reset bit */





struct icm20948_config {
	int (*bus_init)(const struct device *dev);
	const struct i2c_dt_spec i2c;
#ifdef CONFIG_ICM20948_TRIGGER
	const struct gpio_dt_spec int_gpio;
#endif /* CONFIG_ICM20948_TRIGGER */
};



enum icm20948_sensor_channel {
    ICM20948_CHAN_ACCEL_OFFSET_X,
    ICM20948_CHAN_ACCEL_OFFSET_Y,
    ICM20948_CHAN_ACCEL_OFFSET_Z,
    ICM20948_CHAN_GYRO_OFFSET_X,
    ICM20948_CHAN_GYRO_OFFSET_Y,
    ICM20948_CHAN_GYRO_OFFSET_Z,
    ICM20948_CHAN_MAG_OFFSET_X,
    ICM20948_CHAN_MAG_OFFSET_Y,
    ICM20948_CHAN_MAG_OFFSET_Z,
};



struct icm20948_data {
	const struct icm20948_transfer_function *hw_tf;
	int16_t accel_x;
	int16_t accel_y;
	int16_t accel_z;
	int16_t gyro_x;
	int16_t gyro_y;
	int16_t gyro_z;

#if defined(CONFIG_ICM20948_ENABLE_TEMP)
	int16_t temperature;
#endif /* CONFIG_ICM20948_ENABLE_TEMP */

#if defined(CONFIG_ICM20948_ENABLE_MAGN)
	int16_t mag_x;
	int16_t mag_y;
	int16_t mag_z;
#endif /* CONFIG_ICM20948_ENABLE_MAGN */

	/* configuration */
	uint8_t current_bank;
	uint8_t accel_fs_sel;
	uint8_t gyro_fs_sel;
	uint8_t accel_dlpf_cfg;
	uint8_t gyro_dlpf_cfg;

	/* status flags */
	bool accel_dlpf_enable;
	bool gyro_dlpf_enable;

#ifdef CONFIG_ICM20948_TRIGGER
	const struct device *dev;
	struct gpio_callback gpio_cb;

	sensor_trigger_handler_t data_ready_handler;
	const struct sensor_trigger *data_ready_trigger;

#if defined(CONFIG_ICM20948_TRIGGER_OWN_THREAD)
	K_KERNEL_STACK_MEMBER(thread_stack, CONFIG_ICM20948_THREAD_STACK_SIZE);
	struct k_thread thread;
	struct k_sem gpio_sem;
#elif defined(CONFIG_ICM20948_TRIGGER_GLOBAL_THREAD)
	struct k_work work;
#endif
#endif /* CONFIG_ICM20948_TRIGGER */
};

struct icm20948_transfer_function {
	int (*read_reg)(const struct device *dev, uint8_t reg, uint8_t *data, size_t len);
	int (*write_reg)(const struct device *dev, uint8_t reg, uint8_t *data, size_t len);
};



int icm20948_i2c_init(const struct device *dev);
int icm20948_init(const struct device *dev);


bool icm20948_is_connected(const struct device *dev);
uint8_t icm20948_get_who_am_i(const struct device *dev);
int icm20948_data_ready(const struct device *dev);

//power management
int icm20948_sleep(const struct device *dev, bool enable);
int icm20948_low_power(const struct device *dev, bool enable);
int icm20948_set_clock_source(const struct device *dev, uint8_t clock_source);

// trigger 
int icm20948_clear_interrupts(const struct device *dev);
int icm20948_cfg_int_active_low(const struct device *dev, bool active_low);
int icm20948_cfg_int_open_drain(const struct device *dev, bool open_drain);
int icm20948_cfg_int_latch(const struct device *dev, bool latching);
int icm20948_cfg_int_any_read_to_clear(const struct device *dev, bool enabled);

// Individual interrupt enables
int icm20948_int_enable_i2c(const struct device *dev, bool enable);
int icm20948_int_enable_pll(const struct device *dev, bool enable);
int icm20948_int_enable_wom(const struct device *dev, bool enable);
int icm20948_int_enable_raw_data_ready(const struct device *dev, bool enable);
int icm20948_int_enable_fifo_overflow(const struct device *dev, uint8_t bm_enable);
int icm20948_int_enable_fifo_watermark(const struct device *dev, uint8_t bm_enable);


// for low-power applications
int icm20948_wom_logic(const struct device *dev, uint8_t enable, uint8_t mode);
int icm20948_wom_threshold(const struct device *dev, uint8_t threshold);


// FIFO support
int icm20948_enable_fifo(const struct device *dev, bool enable);
int icm20948_reset_fifo(const struct device *dev);
int icm20948_set_fifo_mode(const struct device *dev, bool snapshot);
int icm20948_get_fifo_count(const struct device *dev, uint16_t *count);
int icm20948_read_fifo(const struct device *dev, uint8_t *data, uint16_t len);


// sensor calibration
int icm20948_set_accel_bias(const struct device *dev, int32_t bias_x, int32_t bias_y, int32_t bias_z);
int icm20948_get_accel_bias(const struct device *dev, int32_t *bias_x, int32_t *bias_y, int32_t *bias_z);
int icm20948_set_gyro_bias(const struct device *dev, int32_t bias_x, int32_t bias_y, int32_t bias_z);
int icm20948_get_gyro_bias(const struct device *dev, int32_t *bias_x, int32_t *bias_y, int32_t *bias_z);


// Digital Motion Processor support
int icm20948_enable_dmp(const struct device *dev, bool enable);
int icm20948_reset_dmp(const struct device *dev);
int icm20948_load_dmp_firmware(const struct device *dev);
int icm20948_dmp_set_start_address(const struct device *dev, uint16_t address);


/* Low-level register access functions used across driver files */
int icm20948_read_reg(const struct device *dev, uint8_t bank, uint8_t reg, uint8_t *val);
int icm20948_write_reg(const struct device *dev, uint8_t bank, uint8_t reg, uint8_t val);
int icm20948_read_data(const struct device *dev, uint8_t bank, uint8_t reg, uint8_t *data, size_t len);
int icm20948_write_data(const struct device *dev, uint8_t bank, uint8_t reg, uint8_t *data, size_t len);
int icm20948_update_reg(const struct device *dev, uint8_t bank, uint8_t reg, uint8_t mask, uint8_t value);




#if defined(CONFIG_ICM20948_SENSORHUB)
int icm20948_shub_init_external_chip(const struct device *dev);
int icm20948_shub_read_external_chip(const struct device *dev, uint8_t reg, uint8_t *data, size_t len);
#endif

#ifdef CONFIG_ICM20948_TRIGGER
int icm20948_trigger_set(const struct device *dev, const struct sensor_trigger *trig, sensor_trigger_handler_t handler);
int icm20948_init_interrupt(const struct device *dev);
#endif /* CONFIG_ICM20948_TRIGGER */

extern const struct sensor_driver_api icm20948_api;

#endif /* ZEPHYR_DRIVERS_SENSOR_ICM20948_ICM20948_H_ */